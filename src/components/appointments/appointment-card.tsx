...
export function TurnoCard({ turno, onConfirm, onCancel, onReschedule }: TurnoCardProps) {
  const initials = turno.personalName
    .split(' ')
    .map(n => n[0])
    .join('')
    .toUpperCase()
  ...
  return (
    <Link href={`/turnos/${turno.id}`}>
      <Card variant="gold-accent" className="hover:shadow-card-hover transition-shadow cursor-pointer">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between gap-3 mb-1">
              <h3 className="font-semibold text-foreground truncate">{turno.personalName}</h3>
              <Badge variant={statusVariant[turno.status]}>
                {statusLabels[turno.status]}
              </Badge>
            </div>

            {turno.personalSpecialty && (
              <p className="text-sm text-accent-500 mb-2">{turno.personalSpecialty}</p>
            )}
        ...
        {turno.status === 'pending' && (onConfirm || onCancel || onReschedule) && (
          <div className="mt-4 pt-4 border-t border-border flex gap-3" onClick={e => e.preventDefault()}>
            {onConfirm && (
              <Button size="sm" onClick={() => onConfirm(turno.id)}>
                Confirmar
              </Button>
            )}
            {onReschedule && (
              <Button size="sm" variant="outline" onClick={() => onReschedule(turno.id)}>
                Reagendar
              </Button>
            )}
            {onCancel && (
              <Button size="sm" variant="ghost" onClick={() => onCancel(turno.id)}>
                Cancelar
              </Button>
            )}
          </div>
